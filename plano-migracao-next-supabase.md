...
2026-05-07 · inicio/dashboard · `src/app/page.tsx`, `src/styles/globals.css`, `src/lib/navigation.ts` · home atualizada para parecer um dashboard operacional finalizado, com KPIs reais do Supabase, atalhos para fichas/quadro/relatorios, fila recente e bloco de prontidao da base · decisao: manter `/` como entrada executiva e nao recriar `/dashboard`; os links continuam apontando para as superficies ativas · caveat: a checagem visual ainda deve ser refeita em navegador depois do build/local server.

2026-05-08 · inicio/dashboard · `src/app/page.tsx`, `src/styles/globals.css` · polimento visual aplicado na home atual: saudacao por usuario logado, data local, KPIs usando `Card` com acento semantico, atalhos mais compactos e lista de ultimas fichas com avatar, `Badge` compartilhado e alinhamento consistente do status · decisao: manter o ajuste como refinamento da entrada `/`, sem reabrir rota `/dashboard` nem adicionar microcopy explicativa · validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e Edge em `localhost:3000` autenticado como Fernanda, com badges alinhados e console sem erros.

2026-05-08 · inicio/dashboard · `src/app/page.tsx`, `src/app/layout.tsx`, `src/styles/globals.css` · cumprimento principal corrigido para ficar inline no titulo (`Ola, usuario, boa tarde!`), usando `Playfair Display` apenas no trecho do cumprimento e removendo o paragrafo separado que quebrava a composicao; hotfix carregou o estilo `italic` da Playfair via `next/font` e apontou o CSS diretamente para `--font-serif-display` · decisao: preservar a rota `/` e o carregamento global de fontes · validacao: `npm run typecheck`, `npx eslint src/app/layout.tsx src/app/page.tsx`, `npm run build`, `git diff --check` e Edge em `localhost:3000/?fontcheck=playfair`, com titulo em linha unica e Playfair real no cumprimento.

2026-05-08 · inicio/datas · `src/lib/dates.ts`, `src/app/page.tsx`, `src/app/layout.tsx`, `src/styles/globals.css`, `src/features/fichas/data.ts`, `src/features/fichas/fichas-overview.tsx`, `src/features/relatorios/data.ts`, `src/app/fichas/pdf/route.ts` · home ganhou cumprimento operacional (`Bom dia`, `Boa tarde`, `Boa noite`) em Playfair Display italico, calculado por `America/Cuiaba`; datas operacionais de hoje, semana atual/proxima, atrasadas e periodos de relatorio passaram a consumir helper compartilhado em vez de depender do timezone local do servidor · decisao: manter `date-fns` instalado para calendario/formularios, mas centralizar timezone de negocio via `Intl` porque o projeto nao tem `date-fns-tz` como dependencia direta · validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e `git diff --check` passaram; caveat: smoke HTTP em `localhost:3000` redirecionou para `/login`, entao a conferencia visual autenticada da home ficou para sessao com login.

2026-05-08 · quadro/fichas · `src/features/quadro-producao/config.ts`, `src/features/quadro-producao/data.ts`, `src/features/quadro-producao/quadro-producao-client.tsx`, `src/features/fichas/fichas-overview.tsx`, `src/lib/formatters.ts`, `supabase/migrations/20260508120000_quadro_producao_personalizacao_labels.sql` · nomes das etapas do quadro agora variam por personalizacao: Bordado/Patch/DTF usam fluxo de PDF com etapas intermediarias ocultas, Serigrafia usa cores/fotolito/estamparia, Sublimacao usa exportacao/impressao/sublimado e Todos/Outros usa rotulos combinados · decisao: manter slugs tecnicos persistidos e calcular apenas o rotulo/visibilidade operacional por tipo; a listagem de `/fichas` usa o mesmo helper para refletir a etapa correta por ficha · validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check`, `git diff --check` e Edge em `localhost:3000` para `/quadro-producao?arte=dtf`, `/quadro-producao?arte=serigrafia`, `/quadro-producao?arte=sublimacao` e `/fichas?busca=Larissa`, sem erros de console.

2026-05-08 · fichas/listagem · `src/features/fichas/fichas-overview.tsx`, `src/styles/globals.css` · metadado de etapa ganhou destaque visual na coluna `Etapas`, mantendo a personalizacao como contexto e exibindo a etapa calculada em `Badge` informativo compacto · decisao: reforcar a leitura operacional sem adicionar texto explicativo ou nova coluna · validacao: `npm run typecheck`, `npm run lint`, `git diff --check` e Edge em `localhost:3000/fichas?busca=Larissa`, com console sem erros.

2026-05-08 · fichas/filtros · `src/styles/globals.css` · controles dentro de `.field` agora usam `min-width: 0`, permitindo que os inputs de data respeitem o grid da toolbar sem invadir o `gap` entre `Entrega inicial` e `Entrega final` · decisao: manter o espacamento pelo `gap` padrao da toolbar, sem margem local entre campos · validacao: `npm run typecheck`, `npm run lint`, `git diff --check` e Edge em `localhost:3000/fichas?busca=Larissa`, medindo `gap` visual de 12px e console sem erros.

2026-05-08 · fichas/formulario · `src/components/ui/custom-datalist.tsx`, `src/features/catalogos/data.ts`, `src/features/fichas/ficha-form.tsx` · sugestoes de `CustomDatalist` deixaram de exibir aliases tecnicos/slugs na segunda linha; aliases continuam pesquisaveis, mas a UI mostra apenas detalhes curados como aliases humanos e composicao · decisao: preservar compatibilidade de busca/importacao sem expor termos tecnicos para o operador nas telas de nova ficha/edicao · validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `git diff --check` e Edge em `localhost:3000/fichas/nova`, buscando `malha` e `malha_fria_pv`, com nome/composicao exibidos e console sem erros.

2026-05-08 · shell/usuario · `src/components/ui/app-shell.tsx`, `src/styles/globals.css` · bloco do usuario na sidebar passou a exibir apenas o nome visivel, removendo a linha de perfil `Superadmin`/`Operador` · decisao: manter a autorizacao interna intacta e reduzir microcopy administrativa no shell · validacao: `npm run typecheck`, `npm run lint`, `git diff --check` e Edge em `localhost:3000/fichas?busca=Larissa`, com `.app-user` renderizando somente `Fernanda` e console sem erros.

2026-05-08 - fichas/formulario - `src/features/fichas/ficha-form.tsx`, `src/features/fichas/ficha-save-toast.tsx`, `src/features/fichas/ficha-draft-storage.ts` - criacao de ficha passou a salvar snapshot local no `localStorage` durante a edicao, ao tentar salvar, antes de sair e tambem no desmontar por navegacao interna do shell; erro de salvamento seguido de fechamento da aba mostra em `/fichas/nova` um toast warning persistente com acoes `Restaurar` e `Descartar`, enquanto sucesso em `/fichas?saved=created` remove o snapshot - hotfix: apos teste manual sem toast, o autosave passou a cobrir campos, produtos, imagens e datas com debounce, e o cleanup faz flush antes de cancelar timers - decisao: nao reidratar automaticamente, usar `AlertDialog` para links internos com dados nao salvos e `beforeunload` nativo para fechar/recarregar a aba; arquivos locais ainda nao enviados nao entram no snapshot, mas imagens ja persistidas no Cloudinary entram - validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e scan de encoding nos arquivos tocados.

2026-05-08 - quadro/producao - `src/features/quadro-producao/quadro-producao-client.tsx` - corrigido tratamento client-side de cards durante drag/drop: o estado de arraste agora e compartilhado entre colunas, a coluna de origem deixa de manter uma lista local antiga depois do drop e a movimentacao otimista atualiza todas as queries do quadro antes da revalidacao - decisao: manter a RPC `move_kanban_card` como fonte de persistencia e atacar o flash/duplicacao na borda React Query + `fluid-dnd` - validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check`; caveat: validacao visual do arraste real ficou pendente porque o Edge local redirecionou para `/login?next=/quadro-producao`.

2026-05-08 - quadro/producao - `src/features/quadro-producao/quadro-producao-client.tsx` - hotfix do movimento de cards: os botoes de proxima coluna passaram pelo mesmo fluxo de limpeza do drag/drop e os cards deixaram de usar `motion.article`, evitando disputa de `transform` entre Framer Motion e `fluid-dnd` que deslocava o item para longe do ponteiro - decisao: priorizar previsibilidade do DnD sobre animacao de entrada/saida nos cards - validacao: `npm run typecheck`, `npm run lint`, `npm run build`.

2026-05-08 - quadro/producao - `src/features/quadro-producao/quadro-producao-client.tsx` - adicionados toasts de confirmacao para atualizar o quadro, ordenar coluna por data de entrega e marcar ficha como entregue pelo card/modal - decisao: usar `sonner` direto no client do quadro, sem provider ou shim novo - validacao: `npm run typecheck`, `npm run lint`, `npm run build`.

2026-05-08 - quadro/producao - `src/features/quadro-producao/quadro-producao-client.tsx` - hotfix da entrega pelo card: fichas marcadas como entregues passam a ser ocultadas localmente no quadro no mesmo clique, com rollback se a mutacao falhar, e o contador usa a lista filtrada - decisao: nao depender apenas do refetch para remover o card entregue da superficie atual - validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `git diff --check`.

2026-05-08 - infra/vercel-cdn - `src/proxy.ts`, `src/app/api/cloudinary/config/route.ts`, `src/app/api/cloudinary/signature/route.ts`, `src/app/api/cloudinary/image/[...publicId]/route.ts`, `src/app/fichas/pdf/route.ts`, `src/app/relatorios/excel/route.ts`, `src/features/quadro-producao/data.ts` - reduzido risco de Fast Origin Transfer elevado: o proxy deixou de rodar em route handlers, exports e assets com extensao; as rotas excluidas ganharam checagem direta de sessao; o payload do quadro removeu campos nao consumidos pela UI (`created_at`, `updated_at`, `kanban_status_updated_at`, `observacoes`) - decisao: atacar primeiro middleware desnecessario e bytes de resposta de funcao, mantendo rotas autenticadas sem cache publico - validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e `git diff --check`.
...
- [x] Datas operacionais: centralizar timezone, formatacao e calculos em `src/lib/dates.ts`, incluindo atraso, periodos, ranges semanais e exibicao pt-BR. `date-fns` segue disponivel, mas nao deve ser usado sem passar pela regra de timezone do helper.
...
| 2026-05-06 | DnD/Fluid | Uso correto reforcado | A integracao foi ajustada para seguir melhor o modelo da Fluid: configs de `useDragAndDrop` memoizadas em produtos, imagens e colunas, keys estaveis por id sem indice e callbacks do quadro estabilizados antes de chegar nas listas. Isso evita recriar observers/listeners da biblioteca em renders normais e preserva os nos que a Fluid anima durante reordenacao. Arquivos alterados: `src/features/fichas/ficha-form.tsx`, `src/features/quadro-producao/quadro-producao-client.tsx`, `plano-migracao-next-supabase.md`, `registro-migracao-next.md`. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e smoke no Edge em `/fichas/nova` e `/quadro-producao` sem warnings/erros de console. Caveat: a automacao por JS nao reproduziu um gesto fisico completo de drag, entao ainda vale validar manualmente a sensacao final no browser. |
| 2026-05-06 | DnD/Fluid | Fluidez CSS corrigida | Comparado o exemplo React oficial de listas agrupadas da Fluid com o CSS computado do app: a demo nao anima `transform` nos itens, enquanto produtos e cards do quadro tinham `transition: ... transform ...`, atrasando o movimento que a Fluid aplica por `transform`. `src/styles/globals.css` removeu `transform` das transicoes de itens DnD, removeu translate/rotate decorativos durante hover/drag e adicionou `will-change: transform`. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e Edge confirmando `transformInTransition: false` em `/fichas/nova` e `/quadro-producao`. |
| 2026-05-06 | DnD/Fluid | Kanban entre colunas | O quadro passou a usar o card inteiro como handler da Fluid, deixando a antiga alca apenas como pista visual. A persistencia de drop entre colunas agora acontece no `onDragEnd` da coluna de destino, com `destinationIndex` normalizado para zero quando a Fluid reporta drop no topo como `-1`. Textos/icones/chips nao interativos do card receberam `pointer-events: none` para reduzir alvos nao interativos durante o drag, enquanto botoes e selects continuam clicaveis. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e Edge: card com `handler-class`, drag simulado entre colunas gerando payload `/move` com destino correto e console sem novos erros capturados. |
| 2026-05-06 | DnD/Fluid | Dropzone visual removida | Removida a alca/dropzone visivel dos cards do Kanban, mantendo o card inteiro como area de arraste da Fluid. `src/features/quadro-producao/quadro-producao-client.tsx` deixou de renderizar `.quadro-producao-card__drag-handle` e `src/styles/globals.css` removeu os estilos relacionados; o grip do cabecalho da coluna permanece. |
| 2026-05-06 | DnD/Fluid | Preview bloqueado durante drag | O preview de imagem do card agora ignora hover/focus/click enquanto ha um card em arraste ativo e fecha qualquer preview ao cruzar o botao durante o drag. A regra que deixava o footer de acoes visivel em `.quadro-producao-card--dragging` foi removida para evitar tooltip/acessos acidentais durante o movimento. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e Edge: drag simulado entre colunas manteve 0 previews no DOM e payload `/move` correto. |
| 2026-05-06 | DnD/Fluid | Guard de alvo de evento | O erro persistente `t.closest is not a function` foi rastreado ao bundle da `fluid-dnd@2.6.3`: a lib chama `event.target.closest(...)` em eventos globais de mouse, mas alguns eventos chegam com alvo `Document`/`Node`. Criado `src/lib/fluid-dnd-event-target-guard.ts` e aplicado em fichas/quadro para normalizar apenas eventos de mouse sem alvo `Element`, redisparando-os no elemento sob o ponteiro antes da Fluid processar. |
| 2026-05-06 | Fichas/Listagem | Metadados compactos | `/fichas` passou a buscar `ficha_itens(quantidade)` na listagem, calcular o total de itens por ficha e renderizar `itens` antes de `Venda` como badges neutros pequenos. Arquivos alterados: `src/features/fichas/data.ts`, `src/features/fichas/fichas-overview.tsx`, `src/styles/globals.css`. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e Edge em `/fichas`. |
| 2026-05-07 | Fichas/Imagens e duplicacao | Implementado | `/fichas` agora usa miniaturas Cloudinary de baixa resolucao na listagem via `FichaRowThumbnail`, com spinner enquanto carrega e cursor pointer. Hover/focus abre zoom maior em portal, no estilo do quadro de producao. A previa de impressao ganhou botao `Duplicar`, que abre `/fichas/nova?duplicar=<id>` como rascunho com todos os dados da ficha original exceto imagens; a nova ficha so e criada ao salvar. Arquivos alterados: `src/lib/cloudinary-images.ts`, `src/features/fichas/ficha-row-thumbnail.tsx`, `src/features/fichas/fichas-overview.tsx`, `src/features/fichas/ficha-print-preview-modal.tsx`, `src/features/fichas/ficha-preview.tsx`, `src/features/fichas/ficha-form.tsx`, `src/app/fichas/page.tsx`, `src/app/fichas/nova/page.tsx`, `src/styles/globals.css`. Validado com `npm run typecheck` e eslint dirigido; refinamento visual ficou para uma rodada posterior. |
| 2026-05-07 | Quadro de producao/DnD | Select e duplo clique corrigidos | `src/features/quadro-producao/quadro-producao-client.tsx` passou a bloquear eventos de controles internos do card na fase de captura e com `stopImmediatePropagation`, evitando que o select de status feche por ativar arraste. O titulo do card tambem deixou de iniciar drag em duplo clique, e a lista renderizada deduplica ids antes do render para evitar warning de key duplicada em frames intermediarios da Fluid. Validado com `npm run typecheck` e eslint dirigido. |
...
| 2026-05-08 | Datas/Padronizacao | Ajustado | Regra duravel adicionada ao `AGENTS.md`: datas operacionais novas ou alteradas devem usar `src/lib/dates.ts` e timezone `America/Cuiaba`, preservando `toISOString()` apenas para auditoria/sessao/persistencia tecnica em UTC. Formatadores locais e calculos de data foram migrados em clientes, fichas, relatorios, usuarios, quadro de producao e exportacao Excel. Validado com `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e `git diff --check`. |
