...
2026-05-08 - fichas/formulario - `src/features/fichas/ficha-form.tsx`, `src/features/fichas/ficha-save-toast.tsx`, `src/features/fichas/ficha-draft-storage.ts` - criacao de ficha passou a salvar snapshot local no `localStorage` durante a edicao, ao tentar salvar e antes de sair com alteracoes pendentes; erro de salvamento seguido de fechamento da aba mostra em `/fichas/nova` um toast warning persistente com acoes `Restaurar` e `Descartar`, enquanto sucesso em `/fichas?saved=created` remove o snapshot - hotfix: apos teste manual sem toast, o autosave passou a cobrir campos, produtos, imagens e datas com debounce, sem depender apenas de submit/saida - decisao: nao reidratar automaticamente, usar `AlertDialog` para links internos com dados nao salvos e `beforeunload` nativo para fechar/recarregar a aba; arquivos locais ainda nao enviados nao entram no snapshot, mas imagens ja persistidas no Cloudinary entram - validacao: `npm run typecheck`, `npm run lint`, `npm run build`, `npm run supabase:check` e scan de encoding nos arquivos tocados.
