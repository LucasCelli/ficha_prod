...
export default async function PerfisAdminPage({searchParams}:{searchParams:Promise<Params>}) {
  ...
  const goals=new Map(result.goals.map((g)=>[g.user_id,g]));
  return <section className={styles.page}>
    <header className={styles.header}><div><p className="eyebrow">Administração</p><h1>Metas e autoria</h1><p>Configure metas e transfira a autoria com confirmação e auditoria.</p></div><Link className="ui-button ui-button--secondary" href="/usuarios">Voltar</Link></header>
    <section className={styles.panel}><div className={styles.title}><h2>Metas de {result.month.slice(0,7)}</h2><Badge>{result.users.length}</Badge></div>
      <div className={styles.goals}>{result.users.map((u)=>{const g=goals.get(u.id);return <form action={saveMonthlyGoalAction} key={u.id}><div><strong>{u.display_name}</strong><small>@{u.username}</small></div><input type="hidden" name="userId" value={u.id}/><input type="hidden" name="month" value={result.month}/><label>Fichas<input min="0" name="fichas" type="number" defaultValue={g?.fichas_target??0}/></label><label>Peças<input min="0" name="pieces" type="number" defaultValue={g?.pieces_target??0}/></label><button className="ui-button ui-button--secondary">Salvar</button></form>})}</div>
    </section>
    <section className={styles.panel}>
      <div className={styles.title}><div><h2>Gestão de autoria</h2><p>Selecione uma ou mais fichas, inclusive já atribuídas.</p></div><Badge>{result.total}</Badge></div>
