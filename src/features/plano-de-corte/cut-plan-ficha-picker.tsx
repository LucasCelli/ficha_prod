...
export function CutPlanFichaPicker({ added, onAdd, onRemove }: { added: CutPlanSourceFicha[]; onAdd: (ficha: CutPlanSourceFicha, overwrite?: boolean) => boolean; onRemove: (id: string) => void }) {
  ...
  return <><section aria-labelledby="cut-plan-fichas-title" className="cut-plan-fichas">
    <h3 className="cut-plan__subheading" id="cut-plan-fichas-title">Importar fichas</h3>
    <div className="cut-plan-fichas__search">
      ...
      <Button disabled={loading} onClick={() => void search()} type="button" variant="ghost">{loading ? <span className="button-spinner" aria-hidden="true" /> : <Search size={16} />} Buscar</Button>
      <Button disabled={loading || !selectedId} onClick={() => void add(selectedId)} type="button" variant="secondary"><Plus size={17} /> Adicionar ficha</Button>
    </div>
