...
export function CutPlanPrintSimple({ alternative, input }: { alternative: CutPlanAlternative; input: CutPlanInput }) {
  ...
  return <div className="cut-plan-print-simple">
    <section className="cut-plan-print-simple__page print-page">
      ...
      <dl className="cut-plan-print-simple__summary"><div><dt>Enfestos</dt><dd>{alternative.layCount}</dd></div><div><dt>Total de folhas</dt><dd>{totalLayers}</dd></div></dl>
      {alternative.result.fabrics.map((fabricResult) => { const fabric = input.fabrics.find((item) => item.id === fabricResult.fabricId)!; const showSleeveType = new Set(fabricResult.sizes.map((size) => size.sleeveType)).size > 1; return <section className="cut-plan-print-simple__fabric" key={fabric.id}>
