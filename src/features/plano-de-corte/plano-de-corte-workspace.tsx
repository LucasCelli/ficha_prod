...
import { countLabel, CutPlanCalculationError, formatMarkerLabel } from "./calculator";
...
export function PlanoDeCorteWorkspace({ catalogFabricOptions, catalogSizes }: { catalogFabricOptions: CustomDatalistOption[]; catalogSizes: CatalogSizeForCutPlan[] }) {
  ...
  const input = useMemo<CutPlanInput>(() => ({ tableLengthCm, maxLayers, mergeFabricsInLays, fabrics, items, sizeProfiles, sourceFichaIds: sourceFichas.map((ficha) => ficha.id) }), [tableLengthCm, maxLayers, mergeFabricsInLays, fabrics, items, sizeProfiles, sourceFichas]);
  ...
  function clear() { setTableLengthCm(800); setMaxLayers(50); setMergeFabricsInLays(false); setFabrics([createFabric(defaultFabricName, undefined, defaultFabricSettings)]); setItems([]); setSourceFichas([]); setAlternatives([]); setConfirmClear(false); }
...
function PlanResult({ alternative, fabrics }: { alternative: CutPlanAlternative; fabrics: CutPlanFabric[] }) {
  ...
  return <><dl className="cut-plan__summary"><Metric label="Tecidos" value={alternative.result.fabrics.length} /><Metric label="Enfestos" value={operationalLays.length} /><Metric label="Total de folhas" value={operationalLays.reduce((sum, lay) => sum + lay.layers, 0)} /><Metric label="Peças a mais" value={alternative.result.fabrics.flatMap((fabric) => fabric.sizes).reduce((sum, size) => sum + Math.max(0, size.difference), 0)} /></dl>
    ...
    {alternative.result.fabrics.map((fabricResult) => { const fabric = fabrics.find((entry) => entry.id === fabricResult.fabricId)!; return <div className="cut-plan__check" key={`check-${fabric.id}`}><h4>Conferência — {fabricLabel(fabric)}</h4><ResultTable><thead><tr><th>Tamanho</th><th>Manga</th><th>Pedido</th><th>Vai cortar</th><th>Diferença</th></tr></thead><tbody>{fabricResult.sizes.map((size) => <tr key={`${size.size}-${size.sleeveType}`}><td>{size.size}</td><td>{size.sleeveType === "LONGA" ? "Longa" : "Curta"}</td><td>{size.requested}</td><td>{size.produced}</td><td><span className={size.difference === 0 ? "is-exact" : "is-changed"}>{size.difference > 0 ? "+" : ""}{size.difference}</span></td></tr>)}</tbody></ResultTable></div>; })}</>;
}
