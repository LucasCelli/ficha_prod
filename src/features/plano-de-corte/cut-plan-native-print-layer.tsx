...
export function CutPlanNativePrintLayer({ alternatives, input, onPrinted }: { alternatives: CutPlanAlternative[]; input: CutPlanInput; onPrinted: () => void }) {
  useEffect(() => {
  ...
  return createPortal(<div className="cut-plan-native-print-root">{alternatives.map((alternative) => <CutPlanPrintSimple alternative={alternative} input={input} key={alternative.id} />)}</div>, document.body);
}
