...
type RelatoriosLegadoOverviewProps = {
...
export function RelatoriosLegadoOverview({ filters, result }: RelatoriosLegadoOverviewProps) {
...
function VendedoresList({ vendedores }: { vendedores: RelatorioVendedor[] }) {
  if (vendedores.length === 0) {
    return <p className="dashboard-muted">Nenhum vendedor encontrado no período.</p>;
  }
...
function RankPanel({ id, items, title }: { id: string; items: RelatorioRankItem[]; title: string }) {
  return (
    <section className="relatorios-panel" aria-labelledby={id}>
      ...
      {items.length === 0 ? (
        <p className="dashboard-muted">Nenhum dado encontrado no período.</p>
      ) : (
        <ol className="relatorios-rank-list">
          {items.map((item) => (
            <li key={item.label}>
