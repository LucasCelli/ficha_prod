...
export function PersonalProductivityChart({ data }: { data: SeriesPoint[] }) {
  ...
  return <ChartFrame
    ...
    series={[{ label: "Fichas criadas", points: chartData.map((point) => ({ label: point.label, value: point.total })) }]}
    title="Evolução de fichas criadas no período"
  >
...
export function PersonalStatusChart({ data }: { data: StatusSlice[] }) {
  ...
  return <ChartFrame
    ...
    className="profile-status-chart"
    legend={<ul>{data.map((slice) => <li key={slice.label}><i style={{ background: slice.color }}/><span>{slice.label}</span><strong>{number.format(slice.value)}</strong></li>)}</ul>}
    series={[{ label: "Fichas", points: data.map((slice) => ({ label: slice.label, value: slice.value })) }]}
    title="Distribuição de fichas por status"
  >
